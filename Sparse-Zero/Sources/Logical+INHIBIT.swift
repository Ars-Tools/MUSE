...
extension Logical.INHIBIT.Vector: SparseVector {
	...
	var state: Set<Int> {
		...
		case (1, 1):
			lhs.state.subtracting(rhs.state)
		case (2..., 1):
			(lhs.state.isEmpty ? Set<Int>() : Set<Int>()).subtracting(rhs.state)
		case (1, 2...):
			rhs.state.isEmpty ? lhs.state : .init()
		case (2..., 2...):
...
extension Logical.INHIBIT.Matrix: SparseMatrix {
	...
	var state: Set<SIMD2<Int>> {
		`repeat`(position: lhs.state, count: (rows / lhs.rows, cols / lhs.cols)).subtracting(
