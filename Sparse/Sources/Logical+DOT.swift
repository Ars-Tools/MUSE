...
extension Logical.DOT.Outer: SparseMatrix {
    ...
    @usableFromInline
    var diagonal: V {
        .init(count: min(lhs.count, rhs.count), entry: lhs.entry.intersection(rhs.entry))
    }
    @inlinable
    subscript(row: Int, col: Int) -> Element {
        !lhs.entry.intersection(rhs.entry).isEmpty
    }
    @usableFromInline
    subscript(row: some RangeExpression<Int>, col: Int) -> V {
        let lhs = lhs[row]
        return.init(count: lhs.count, entry: rhs[col] ? .init() : lhs.entry)
    }
    @usableFromInline
    subscript(row: Int, col: some RangeExpression<Int>) -> V {
        let rhs = rhs[col]
        return.init(count: rhs.count, entry: lhs[row] ? .init() : rhs.entry)
    }
    @usableFromInline
    ...
    var entry: Set<SIMD2<Int>> {
        .init(product(lhs.entry, rhs.entry))
    }
}
extension Logical.DOT.DD: SparseVector {
    ...
    @usableFromInline
    var entry: Set<Int> {
        let lhs = lhs.entry.reduce(into: Dictionary<Int, Set<Int>>()) {
            $0[$1.x, default: .init()].insert($1.y)
        }
        let rhs = rhs.entry.reduce(into: Dictionary<Int, Set<Int>>()) {
...
extension Logical.DOT.MV: SparseVector {
    ...
    var entry: Set<Int> {
        ...
        return.init(rhs.entry.lazy.flatMap {
            lhs[$0, default: .init()]
        })
    }
}
extension Logical.DOT.VM: SparseVector {
    ...
    var entry: Set<Int> {
        ...
        return.init(lhs.entry.lazy.flatMap {
            rhs[$0, default: .init()]
        })
    }
}
extension Logical.DOT.VV: SparseVector {
    ...
    var entry: Set<Int> {
        switch self {
        case.DD(let x, let y):
            Logical.DOT.DD(lhs: x, rhs: y).entry
        case.MV(let x, let y):
            Logical.DOT.MV(lhs: x, rhs: y[0..., 0]).entry
        case.VM(let x, let y):
            Logical.DOT.VM(lhs: x[0, 0...], rhs: y).entry
        }
    }
}
extension Logical.DOT.MM: SparseMatrix {
    ...
    @usableFromInline
    subscript(row: Int, col: Int) -> Element {
        !lhs[row, 0...].entry.intersection(rhs[0..., col].entry).isEmpty
    }
    @usableFromInline
    subscript(row: some RangeExpression<Int>, col: Int) -> V {
...
public func •(_ lhs: some SparseVector<Bool>, _ rhs: some SparseVector<Bool>) -> Bool {
    ...
    return !lhs.entry.intersection(rhs.entry).isEmpty
}
