...
extension Logical.AND.Vector: SparseVector {
    ...
    var entry: Set<Int> {
        ...
        case (1, 1):
            lhs.entry.intersection(rhs.entry)
        case (2..., 1):
            lhs.entry.isEmpty ? .init() : rhs.entry
        case (1, 2...):
            rhs.entry.isEmpty ? .init() : lhs.entry
        case (2..., 2...):
...
//    }
    ...
    var entry: Set<SIMD2<Int>> {
        `repeat`(position: lhs.entry, count: (rows / lhs.rows, cols / lhs.cols)).intersection(
