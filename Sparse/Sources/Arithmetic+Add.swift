...
@usableFromInline
struct AddVector<Element: Numeric, LHS: SparseVector<Element>, RHS: SparseVector<Element>> {
	@usableFromInline let lhs: LHS
	@usableFromInline let rhs: RHS
}
extension AddVector: SparseVector {
	@usableFromInline typealias R = Array<Element>
	@usableFromInline typealias S = AddVector<Element, LHS.S, RHS.S>
	@usableFromInline typealias T = AddVector<Element, LHS.T, RHS.T>
	@usableFromInline typealias U = Element
	@usableFromInline typealias V = AddVector<Element, LHS.V, RHS.V>
	@inlinable
	var count: Int {
...
@usableFromInline
@frozen struct AddMatrix<Element: Numeric, LHS: SparseMatrix<Element>, RHS: SparseMatrix<Element>> {
	@usableFromInline let lhs: LHS
	@usableFromInline let rhs: RHS
}
extension AddMatrix: SparseMatrix {
	@usableFromInline typealias R = Array<Element>
	@usableFromInline typealias S = AddMatrix<Element, LHS.S, RHS.S>
	@usableFromInline typealias T = AddMatrix<Element, LHS.T, RHS.T>
	@usableFromInline typealias U = Element
	@usableFromInline typealias V = AddVector<Element, LHS.V, RHS.V>
	@inlinable
	var rows: Int {
	...
	func lil(for layout: MemoryStrategy) -> (MemoryStrategy, Array<LazyMapSequence<LazyFilterSequence<LazyMapSequence<Dictionary<Int, Element>, Optional<(Int, Element)>>>, (Int, Element)>>) {
		...
		case (.columnMajor, (.rowMajor, let lhs), (.rowMajor, let rhs)), (.rowMajor, (.rowMajor, let lhs), (.rowMajor, let rhs)):
			(.rowMajor, zip(rhs.lazy.map(Dictionary.init(uniqueKeysWithValues:)), lhs).map(+))
