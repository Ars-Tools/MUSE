...
@usableFromInline
@frozen struct PaddedMatrix<Source: SparseMatrix> {
...
extension PaddedMatrix: SparseMatrix {
	...
	@usableFromInline
	func lil(for layout: MemoryStrategy) -> (MemoryStrategy, Array<L>) {
		switch source.lil(for: layout) {
		case (.rowMajor, let lil):
			return (.rowMajor,
			 repeatElement(L.empty, count: t) +
			 lil.map { L.exist($0.lazy.map { ($0 + l, $1) }) } +
			 repeatElement(L.empty, count: b))
		case (.columnMajor, let lil):
			return (.columnMajor,
			 repeatElement(L.empty, count: l) +
			 lil.map { L.exist($0.lazy.map { ($0 + t, $1) }) } +
			 repeatElement(L.empty, count: r))
		}
	}
}
extension PaddedMatrix {
	...
	enum L {
		case empty
		case exist(LazyMapSequence<Source.LIL.Element, (Int, Source.Element)>)
	}
}
extension PaddedMatrix.L: Sequence {
	@usableFromInline
	@frozen struct Iterator: IteratorProtocol {
		@usableFromInline
		typealias RawValue = Optional<LazyMapSequence<Source.LIL.Element, (Int, Source.Element)>.Iterator>
		@usableFromInline
		var rawValue: RawValue
		@inlinable
		mutating func next() -> Optional<(Int, Source.Element)> {
			rawValue?.next()
		}
		@inlinable
	...
	func makeIterator() -> Iterator {
		...
		case.empty:
			.init(rawValue: .none)
		case.exist(let lil):
			.init(rawValue: .some(lil.makeIterator()))
		}
	}
}
public func padding<Element>(_ source: some SparseMatrix<Element>, rows: (Int, Int), cols: (Int, Int)) -> some SparseMatrix<Element> {
