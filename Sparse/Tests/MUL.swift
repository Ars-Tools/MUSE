...
struct MulTestCases {
	...
	func and() {
		...
		let z = x && y
		let w = MSK(z)
		#expect(z[0..<3, 0..<3].state == w[0..<3, 0..<3].state)
		for row in 0..<z.rows {
			let e = x[row, 0...] && y[0, 0...]
			#expect(z[row, 0...].state == e.state)
			#expect(w[row, 0...].state == e.state)
		}
		for col in 0..<z.cols {
			let e = x[0..., 0] && y[0..., col]
			#expect(z[0..., col].state == e.state)
			#expect(w[0..., col].state == e.state)
		}
		for (row, col) in product(0..<z.rows, 0..<z.cols) {
