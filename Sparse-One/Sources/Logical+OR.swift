...
extension Logical.OR.Vector: SparseVector {
    ...
    var entry: Set<Int> {
        ...
        case (1, 1):
            lhs.entry.union(rhs.entry)
        case (2..., 1):
            lhs.entry.isEmpty ? rhs.entry : .init(0..<count)
        case (1, 2...):
            rhs.entry.isEmpty ? lhs.entry : .init(0..<count)
        case (2..., 2...):
...
extension Logical.OR.Matrix: SparseMatrix {
    ...
    var entry: Set<SIMD2<Int>> {
        `repeat`(position: lhs.entry, count: (rows / lhs.rows, cols / lhs.cols)).union(
