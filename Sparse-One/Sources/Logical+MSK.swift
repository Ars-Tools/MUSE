...
extension Logical.MSK.Vector: SparseVector {
    ...
    var entry: Set<Int> {
        ...
        case (1, 1):
            maskee.entry.intersection(masker.entry)
        case (2..., 1):
            maskee.entry.isEmpty ? .init() : masker.entry
        case (1, 2...):
            masker.entry.isEmpty ? .init() : maskee.entry
        case (2..., 2...):
            maskee.entry.isEmpty || masker.entry.isEmpty ? .init() : .init(0..<count)
        default:
            .init()
        }
    }
...
extension Logical.MSK.Matrix: SparseMatrix {
    ...
    func lil(for layout: MemoryStrategy) -> (MemoryStrategy, Array<Optional<LazyMapSequence<LazyFilterSequence<LazyMapSequence<Dictionary<Int, Element>, Optional<(Int, Element)>>>, (Int, Element)>>>) {
        let (row, col) = `repeat`(position: masker.entry, count: (rows / masker.rows, cols / masker.cols)).reduce(into: (Set<Int>(), Set<Int>())) {
            $0.0.insert($1.x)
            $0.1.insert($1.y)
        }
...
extension SparseMatrix where Element: Numeric {
    ...
    public subscript(_ row: some SparseVector<Bool>, _ col: some SparseVector<Bool>) -> some SparseMatrix<Element> {
        Logical.MSK.Matrix(maskee: self, masker: MSK(rows: row.count, cols: col.count, entry: .init(product(row.entry, col.entry))))
    }
    public subscript(_ row: some SparseVector<Bool>, _ col: some RangeExpression<Int>) -> some SparseMatrix<Element> {
        ...
        return Logical.MSK.Matrix(maskee: self, masker: MSK(rows: row.count, cols: col.count, entry: .init(product(row.entry, col))))
    }
    public subscript(_ row: some RangeExpression<Int>, _ col: some SparseVector<Bool>) -> some SparseMatrix<Element> {
        ...
        return Logical.MSK.Matrix(maskee: self, masker: MSK(rows: row.count, cols: col.count, entry: .init(product(row, col.entry))))
    }
}
extension DOK {
    public subscript(_ masker: some SparseMatrix<Bool>) -> DOK<Element> {
        .init(rows: rows, cols: cols, store: .init(uniqueKeysWithValues: masker.entry.compactMap {
            switch store[$0] {
            case.none,.some(.zero):
                .none
            case.some(let v):
