...
extension Logical.INHIBIT.Vector: SparseVector {
    @inlinable
    var count: Int {
    ...
    var entry: Set<Int> {
        ...
        case (1, 1):
            lhs.entry.subtracting(rhs.entry)
        case (2..., 1):
            (lhs.entry.isEmpty ? Set<Int>() : Set<Int>()).subtracting(rhs.entry)
        case (1, 2...):
            rhs.entry.isEmpty ? lhs.entry : .init()
        case (2..., 2...):
            lhs.entry.isEmpty || rhs.entry.isEmpty ? .init() : .init(0..<count)
        default:
            .init()
        }
    }
}
extension Logical.INHIBIT.Matrix: SparseMatrix {
    @inlinable
    var rows: Int {
    ...
    @usableFromInline
    var entry: Set<SIMD2<Int>> {
        `repeat`(position: lhs.entry, count: (rows / lhs.rows, cols / lhs.cols)).subtracting(
            `repeat`(position: rhs.entry, count: (rows / rhs.rows, cols / rhs.cols))
        )
    }
}
public func inhibit(_ lhs: some SparseVector<Bool>, _ rhs: some SparseVector<Bool>) -> some SparseVector<Bool> {
    Logical.INHIBIT.Vector(lhs: lhs, rhs: rhs)
}
public func inhibit(_ lhs: some SparseMatrix<Bool>, _ rhs: some SparseMatrix<Bool>) -> some SparseMatrix<Bool> {
    Logical.INHIBIT.Matrix(lhs: lhs, rhs: rhs)
}
